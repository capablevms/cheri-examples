...
const char* abi_desc(enum ABI abi) {
	switch (abi) {
		case AbiA: return "C64-Secure, with naive (insecure) csp bounds restriction.";
		case AbiB: return "C64-Secure, with conservative csp bounds restriction.";
		case AbiC: return "C64-Secure v2, with precisely-restricted csp (and full cfp).";
		case AbiZ: return "AAPCS64-cap, used by C code.";
		case AbiUnknown: return "Unknown";
	}
}
...
int backtrace(struct Context frame, struct Context const* current) {
	...
	for (Slot slot = cfp_max_slot; slot >= csp_slot; slot--) {
		...
		const char* lbracket =
			lbracket_for(slot, csp_slot, cfp_slot, cfp_max_slot);
		...
		printf(" %s %s",
		       rbracket_for(slot, cur_csp_min_slot, cur_csp_slot, cur_csp_max_slot),
		       (slot == cur_csp_slot) ? "csp" : "   ");
		printf(" %s %s",
		       rbracket_for(slot, cur_cfp_min_slot, cur_cfp_slot, cur_cfp_max_slot),
