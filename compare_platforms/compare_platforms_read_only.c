/*
 * This version can be run in both a CHERI and a non-CHERI environment to compare the two.
 *
 * In a CHERI environment, it will:
 * Step 1: Use capabilities to enforce read-only permissions
 * (see https://github.com/capablevms/cheri-examples/blob/master/employee/include/employee.h).
 * Step 2: Either:
 * a) if command-line flag "-overwrite" was passed in, try to write to the read-only struct members,
 * and SIGPROT; or b) try to exceed the bounds of an object and add write permissions (which will
 * also SIGPROT).
 *
 * In a non-CHERI environment, it will:
 * Step 1: Use can_write to check if the struct's "permissions" member includes write permissions.
