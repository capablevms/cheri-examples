 ...
 *     Step 1: Use can_read to check if the struct's "permissions" member includes write
 * permissions. Step 2: Use capabilities to enforce read-only permissions (see
 * https://github.com/capablevms/cheri-examples/blob/master/employee/include/employee.h). Step 3:
 * Either: a) if command-line flag "-overwrite" was passed in, try to write to the read-only struct
 * members, and SIGPROT; or b) try to exceed the bounds of an object and add write permissions
 * (which will also SIGPROT).
 *
 * In a non-CHERI environment, it will:
 *     Step 1: Use can_read to check if the struct's "permissions" member includes write
 * permissions. Step 2: Try to exceed the bounds of an object and add write permissions (see
 * https://ctsrd-cheri.github.io/cheri-exercises/exercises/buffer-overflow-heap/index.html). Step 3:
 * Rewrite the review.
 */
...
void assert_positions(char *realname, char *publicreview, char *permissions, size_t smsz,
					  size_t bigsz)
{
...
int main(int argc, char *argv[])
{
	...
	strcpy(publicreview,
		   "I am a little unclear as to the contribution. I think the authors could strengthen "
		   "their case considerably if they conducted an RCT. Weak reject.");
	...
	strcpy(newpublicreview,
		   "5 ***** strong accept. This author deserves two Nobels and an ice cream.\n");
	...
	{
		printf("\nThe struct is read-only so trying to change the review will make the program "
			   "crash.\n");
		bWeak = false;
